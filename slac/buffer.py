...
class ReplayBuffer:
    ...
    def sample_latent(self, batch_size):
        ...
        tactile_ = torch.tensor(tactile_, dtype=torch.float32, device=self.device).float().div_(self.force_norm)
    ...
    def misalign_sample_latent(self, batch_size):
        ...
        state_ = torch.tensor(state_, dtype=torch.uint8, device=self.device).float().div_(255.0)
        tactile_ = torch.tensor(tactile_, dtype=torch.float16, device=self.device).float().div_(self.force_norm)
