...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim=3, tactile_dim=6, img_dim=256, tactile_latent_dim=96, hidden_img=256):
        ...
        self.net = nn.Sequential(
            ...
            # (128, 6, 6) -> (256, 3, 3)
            nn.GELU(),
            nn.Conv2d(256, hidden_img, 3),
            nn.GELU(),
        )

        self.tactile_net = nn.Sequential(nn.Linear(tactile_dim, tactile_latent_dim//2),
                                         nn.GELU(),
                                         nn.Linear(tactile_latent_dim//2, tactile_latent_dim),
                                         nn.GELU(),
                                         )
...
class LatentModel(torch.jit.ScriptModule):
    ...
    def calculate_loss(self, state_, tactile_, action_, reward_, done_):
        ...
        feature_, contact_check,  alignment_check= self.encoder(state_,tactile_)
        ...
        contact_sign = torch.where(contact_mag > 0.015, contact, non_contact)
        contact_loss = self.BCE_loss(contact_check, contact_sign)

        aligned = torch.ones_like(alignment_check)
        alignment_loss = self.BCE_loss(alignment_check, aligned)
        ...
        return loss_kld, loss_image, loss_reward, contact_loss, alignment_loss

    def calculate_alignment_loss(self, state_, tactile_):
        feature_, _, alignment_check = self.encoder(state_, tactile_)
        aligned = torch.zeros_like(alignment_check)
        alignment_loss = self.BCE_loss(alignment_check, aligned)
